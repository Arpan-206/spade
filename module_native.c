...
{
    if (spade_state.press_cb)
        jerry_release_value(spade_state.press_cb);
...
{
    if (spade_state.frame_cb)
        jerry_release_value(spade_state.frame_cb);
    ...
    const jerry_length_t argc)
{
    ...
    if (!s)
        return jerry_create_error_sprite_freed();
    ...
    const jerry_length_t argc)
{
    Sprite *s = sprite_from_jerry_object(this_obj);
    if (s)
        return jerry_create_number(s->dx);
    else
        return jerry_create_error_sprite_freed();
}
    ...
    const jerry_length_t argc)
{
    Sprite *s = sprite_from_jerry_object(this_obj);
    if (s)
        return jerry_create_number(s->dy);
    else
        return jerry_create_error_sprite_freed();
}
